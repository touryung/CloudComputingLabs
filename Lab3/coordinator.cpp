...
void coordinator::start() {
  ...
  while (1) {
    ...
    if (command.Command_name == "GET") {
      ...
      while (succeed_participant.find(id) == succeed_participant.end()) id++;
      cout << "id=" << id << endl;
      send_message(id, s);
      ...
      if (recv_message("prepared", n, 500000) == n) {
        ...
        if (recv_message("ACK", n, 500000) < n) {
          ...
          if (recv_message("rollback_successed", n, 500) < n) ProcessTimeout();
        } else {
          string s = "+OK\r\n";
          send_client_message(conn_sock, s);
          continue;
...
void coordinator::recovery() {
  while (1) {
    ...
    for (int i = 0; i < participants_commit_ID.size(); i++) {
      if (participants_commit_ID[i] > maxID) maxID = participants_commit_ID[i];
    }
    if (maxID == -1) return;
    queue<int> good;
    queue<int> bad;
