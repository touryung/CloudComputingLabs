...
static const char *ip_format =
    "^([0-9]|[1-9][0-9]|1[0-9]{1,2}|2[0-4][0-9]|25[0-4]).([0-9]|[1-9][0-9]|1[0-"
    "9]{1,2}|2[0-4][0-9]|25[0-5]).([0-9]|[1-9][0-9]|1[0-9]{1,2}|2[0-4][0-9]|25["
    "0-5]).([1-9]|[1-9][0-9]|1[0-9]{1,2}|2[0-4][0-9]|25[0-5])$";
...
bool get_name_id(char *str, int len, char *name, char *id) {
  int spite = 0;
  while (spite < len && str[spite] != '&') ++spite;
  if (spite == len) return false;

  // 获取name
  int equal1 = 0;
  while (equal1 < spite && str[equal1] != '=') ++equal1;
  if (!(equal1 == 4 && str[0] == 'N' && str[1] == 'a' && str[2] == 'm' &&
        str[3] == 'e'))
    return false;
  int index1 = 0;
  ...
  // 获取id
  int equal2 = spite + 1;
  while (equal2 < len && str[equal2] != '=') ++equal2;
...
int get_line(int sock, char *buf, int size) {
  ...
  int n;
  while ((i < size) && (c != '\n')) {
    // 然后就会跳到else 跳出循环 如果没有数据就返回0 设置定时器
